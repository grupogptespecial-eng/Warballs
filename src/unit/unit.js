import { CFG, CLASSES, CLASS_VISUALS, CLASS_ITEM_SCALE, LEVEL_SAFE_RADIUS } from '../config/cfg.js';
...
export class Unit {
  ...
  drawClassItem(ctx) {
    ...
    const ang = cfg.anchorAngleDeg * Math.PI / 180;
    const ux = Math.cos(ang);
    const uy = Math.sin(ang);

    let dist = this.bodyR * 0.82;
    let cx = this.pos.x + ux * dist;
    let cy = this.pos.y + uy * dist;

    const safe = LEVEL_SAFE_RADIUS;
    const d = Math.hypot(cx - this.pos.x, cy - this.pos.y);
    if (d < safe) {
      dist = safe;
      cx = this.pos.x + ux * dist;
      cy = this.pos.y + uy * dist;
    }

    const diam = this.bodyR * 2;
    const size = (cfg.scale ? cfg.scale * diam : CLASS_ITEM_SCALE);

    const t = performance.now() / 1000;
    let offX = 0, offY = 0, scl = 1, alpha = 1;
    switch (cfg.microAnim) {
      case 'sway_low': {
        const amp = 2, per = 2;
        const s = Math.sin(t / per * Math.PI * 2) * amp;
        offX = -uy * s; offY = ux * s;
        break;
      }
      case 'idle_breath': {
        const amp = 0.02, per = 2.5;
        scl = 1 + Math.sin(t / per * Math.PI * 2) * amp;
        break;
      }
      case 'subtle_pulse': {
        const amp = 0.04, per = 1.6;
        scl = 1 + Math.sin(t / per * Math.PI * 2) * amp;
        break;
      }
      case 'soft_glow': {
        const per = 0.8;
        alpha = 0.6 + Math.sin(t / per * Math.PI * 2) * 0.15;
        break;
      }
      case 'orbit_slow': {
        const per = 6, r = 14;
      ...
      case 'glint_slow': {
        // glint será sugerido com pequena variação de alpha
        const per = 8;
        alpha = 0.8 + Math.sin(t / per * Math.PI * 2) * 0.2;
        break;
      }
    }
    ...
    ctx.lineWidth = 1.2;
    ctx.strokeStyle = '#000';
    ctx.fillStyle = cfg.palette ? cfg.palette[0] : '#fff';

    switch (cfg.item) {
      case 'aljava_pequena': {
        ctx.beginPath();
        ctx.rect(-size * 0.3, -size * 0.5, size * 0.6, size);
        ctx.fill();
        ctx.stroke();
        ctx.strokeStyle = cfg.palette[1] || '#000';
        ctx.lineWidth = 1;
        ctx.beginPath();
      ...
      case 'rosario_punho': {
        for (let i = 0; i < 6; i++) {
          const a = i * Math.PI / 3;
          ctx.beginPath();
          ctx.arc(Math.cos(a) * size * 0.4, Math.sin(a) * size * 0.4, size * 0.12, 0, Math.PI * 2);
          ctx.fill();
          ctx.stroke();
        }
        break;
      ...
      case 'tunica_fina': {
        // pequena tira de pano triangulada
        ctx.fillStyle = cfg.palette[0] || '#fff';
        ctx.beginPath();
      ...
      case 'ombreira_metal': {
        // ombreira semicircular com rebite
        ctx.beginPath();
        ctx.arc(0, 0, size * 0.5, Math.PI * 0.1, Math.PI * 1.1);
        ctx.lineTo(0, 0);
        ctx.closePath();
        ctx.fill();
        ctx.stroke();
        // rebite central
        ctx.fillStyle = cfg.palette[1] || '#000';
        ctx.beginPath();
  ...
  draw(ctx) {
    ...
    if (this.className === 'paladino') {
      ...
      ctx.beginPath();
      ctx.rect(this.bodyR, -swordW / 2, this.weaponLen, swordW);
      ctx.fill();
      ctx.fillStyle = shade(base, -0.4);
      ctx.beginPath();
      ctx.rect(this.bodyR - guardH / 2, -guardW / 2, guardH, guardW);
      ctx.fill();
    } else if (this.className === 'barbaro') {
      ctx.save();
      ctx.translate(this.bodyR + 18, 0);
      ctx.rotate(Math.PI / 4);
      // cabo
      drawRoundedRect(ctx, -2, -18, 4, 36, 2);
      ctx.fillStyle = weaponCol;
      ctx.fill();
      ctx.strokeStyle = shade(weaponCol, -0.2);
      ctx.stroke();
      ...
      ctx.quadraticCurveTo(-22, 10, -8, 0);
      ctx.closePath();
      ctx.fill();
      ctx.stroke();
      // lâmina direita
      ...
      ctx.beginPath();
      ctx.moveTo(ax, ay);
      ctx.lineTo(tx - this.weaponTipR, ty);
      ctx.stroke();
      ctx.beginPath();
      ...
      if (this.gw && this.gw.state === 'THROW_FLIGHT') {
        ...
        ctx.beginPath();
        ctx.moveTo(ax, ay);
        ctx.lineTo(tx - this.weaponTipR - 6, ty);
        ctx.stroke();
        ctx.beginPath();
      ...
      const ax = this.bodyR;
      const ay = 0;
      const tx = this.bodyR + this.weaponLen;
      const ty = 0;
      ctx.lineCap = 'round';
      ctx.strokeStyle = weaponCol;
      ctx.lineWidth = 3;
      ...
      ctx.stroke();
      ctx.beginPath();
      ctx.arc(tx, ty, this.weaponTipR, 0, Math.PI * 2);
      ctx.fillStyle = base;
      ctx.fill();
      ctx.globalAlpha = 0.35;
      ctx.beginPath();
      ctx.arc(tx, ty, this.weaponTipR + 1.8, 0, Math.PI * 2);
      ctx.fillStyle = '#ffffff';
      ctx.fill();
      ctx.globalAlpha = 1;
    }
