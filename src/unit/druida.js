...
const TRAILS = {
  fire:  { color: '#ff6b00', width: 5 },
  ice:   { color: '#7dd3fc', width: 5 },
  lightning: { color: '#ffe45e', width: 4 },
  earth: { color: '#bef264', width: 5 }
};
...
export function fireDruida() {
  ...
  proj.element = elem;
  if (S.projectile) {
    if (S.projectile.scale !== undefined) proj.scale = S.projectile.scale;
    if (S.projectile.internalRotation !== undefined) proj.internalRotation = S.projectile.internalRotation;
  }
  const trail = TRAILS[elem];
  proj.trailColor = trail.color;
  proj.trailWidth = trail.width;
  proj.trail.push(off.clone());
  if (elem === 'fire') {
