    ...
    for (const u of units) {
      ...
      if (d < u.bodyR + this.rad) {
        ...
        if (dealt > 0) {
          ...
          const cols = ['#7F3EF3', '#8be9fd', '#1d4ed8', '#34c14a'];
          const col = cols[Math.floor(Math.random() * cols.length)];
          game.spawnEffect(new Effect(this.pos.clone(), 40, 0.5, col));
          for (let i = 0; i < 2; i++) {
            for (const c of cols) {
              const dir = V.fromAng(Math.random() * Math.PI * 2, rrand(80, 180));
              game.spawnParticle(new Particle(this.pos.clone(), dir, 0.35, c));
            }
          }
        }
        this.pierce--;
