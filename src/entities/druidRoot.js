...
export class DruidRoot {
  constructor(owner, pos, startLevel = 1) {
    ...
    this.pulseHeal = CFG.druida.root.pulse.heal || 0;
    this.vineDps = CFG.druida.root.vineDps || 0;
    this.vineTick = 1; // apply vine damage once per second to avoid XP abuse
  ...
  hit(dmg, attacker) {
    ...
    if (dealt > 0 && attacker?.gainXPOffense) attacker.gainXPOffense(dealt);
  ...
  update(dt, arena, units) {
    ...
    if (this.target && this.target.alive) {
      ...
      if (dist > this.radius + this.target.bodyR) {
        // Target escaped; only the foe takes damage
        const esc = CFG.druida.root.escapeDmg || 0;
        if (esc > 0) this.target.hit(esc, new V(0, 0), this.owner);
        ...
        // periodic vine damage based on nodule level
        this.vineTick -= dt;
        if (this.vineTick <= 0) {
          this.vineTick += 1;
          const dmg = this.vineDps;
    ...
    if (this.pulseCD <= 0) {
      ...
      for (const u of units) {
        ...
        if (d.len() <= this.pulseRadius + u.bodyR) {
          if (this.team && u.team && this.team === u.team) {
            if (this.pulseHeal > 0 && this.hp > 0) {
              const amt = Math.min(this.pulseHeal, u.hpMax - u.hp, this.hp);
              if (amt > 0) { u.hp += amt; this.hp -= amt; }
            }
          } else {
