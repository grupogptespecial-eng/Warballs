...
export function drawWeapon(ctx, unit){
  ...
  let scaleFactor = wv.scale ?? 1;
  if (wv.weaponReach != null) {
    scaleFactor *= wv.weaponReach / base;
  }
  // Accept legacy or explicit angle fields (anchorAngleDeg / weaponAngleDeg)
  const aDeg = wv.anchorAngleDeg ?? wv.weaponAngleDeg ?? wv.anchorDeg ?? 0;
  ...
  const drawOne = (angOff, which) => {
    ctx.save();
    ctx.translate(unit.pos.x, unit.pos.y);
    ctx.rotate(anchorAng + angOff);
    ctx.translate(dist, 0);
    ctx.rotate(iRot);
    ctx.scale(scaleFactor, scaleFactor);
    if (anchor[0] || anchor[1]) ctx.translate(-anchor[0] * base, -anchor[1] * base);
    drawFn(ctx, base, pal, unit, which);
    ctx.restore();
  };

  if (unit.className === 'ladino') {
    drawOne(0,0);
    drawOne(Math.PI,1);
  } else {
    drawOne(0,0);
